...
main(int argc, char *argv[])
{
  ...
  int sdmax; /* largest file descriptor number*/
  ...
  while (1) {
    ...
    fdset = livesdset;
    FD_SET(servsock, &fdset);//combine livesdset and servsdset
	...
	if(FD_ISSET(frsock, &livesdset)/* TODO: input from existing client? */) {
		...
		/* TODO: insert newsd into fd_set(s) */
                ...
                FD_SET(newsd, &fdset);
                if(newsd > sdmax){ /* update the largest file descriptor number */
                    sdmax = newsd;
                }
	    }
	} 
	...
	if (csd != -1) {
	    /* TODO: put csd into fd_set(s) */
            if(csd > sdmax){ /* update the largest file descriptor number*/
                sdmax = csd;
            }
